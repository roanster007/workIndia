...
def process_booking_information(data):
    ...
    with transaction.atomic():
        ...
        is_seats_available = check_seat_availability(
            train_id, source, destination, seats
        )
...
def check_seat_availability(train_id, source, destination, seats):
    total_seats = (
        Train.objects.filter(id=train_id).values_list("seats", flat=True).first()
    )
